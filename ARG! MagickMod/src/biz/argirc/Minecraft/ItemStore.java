...
public class ItemStore {
	...
	public Boolean getCurrencyTotal(CommandSender sender) {
		...
		if (hasaccount == false) {
			Accounting.write(player, EconomySettings.startingbalance);
		}
		int balance = Accounting.getBalance(player);
		player.sendMessage(ChatColor.GREEN + "You have " + balance + " " + EconomySettings.credit);
		return true;
	...
	public Boolean buyCommand(CommandSender sender, String[] args) {
		...
		if (Accounting.containskey(player)) {
			boolean IsForSale = ItemStore.containsItem(item, plugin.ItemStoreFile);

			if (IsForSale) {
				int Price = ItemStore.getPrice(item, plugin.ItemStoreFile);
				int count = ItemStore.getCount(item, plugin.ItemStoreFile);
				int ItemID = ItemStore.getItemID(item, plugin.ItemStoreFile);
				int balance = Accounting.getBalance(player);
				...
				if (balance - Price >= 0) {
					if (player.getInventory().firstEmpty() == -1) {
						...
						player.sendMessage(ChatColor.GREEN + "->You got " + count + " " + item + " for " + Price + " " + EconomySettings.credit);
						player.sendMessage(ChatColor.GREEN + " ->You have " + newbalance + " " + EconomySettings.credit + " left");
						return true;
					}
				} else {
					player.sendMessage(ChatColor.RED + "You need more " + EconomySettings.credit + " man.");
					return false;
				}
			}
		}
	...
	public boolean getStore(Player p) {
		...
		try {
			FileInputStream in = new FileInputStream(plugin.ItemStoreFile);
			pro.load(in);
			@SuppressWarnings("rawtypes")
			Enumeration e = pro.propertyNames();
			while (e.hasMoreElements()) {
				String key = (String) e.nextElement();
				int count = ItemStore.getCount(key, plugin.ItemStoreFile);
				int price = ItemStore.getPrice(key, plugin.ItemStoreFile);

				p.sendMessage(count + " " + key + " for " + price + " " + EconomySettings.credit);
			}
			return true;
		} catch (IOException e) {
			return false;
